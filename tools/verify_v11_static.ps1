...
func\s)[\s\S])*?var\s+center\s*:=\s*info_back_center\(panel_center\)'
	...
	@{
		...
		Pass = $extraHud -match 'var\s+energy_slots:\s*Array\[DIRExtraEnergySlot\]' -and $extraHud -match 'for\s+_i\s+in\s+4:' -and $extraHud -match 'func\s+update_energy\((?:(?!\r?\nfunc\s)[\s\S])*?clampi\(quarter_units\s*-\s*i\s*\*\s*4,\s*0,\s*4\)(?:(?!\r?\nfunc\s)[\s\S])*?float\(slot_quarter_units\)\s*/\s*4\.0' -and $extraEnergySlot -match 'class_name\s+DIRExtraEnergySlot' -and $extraEnergySlot -match 'var\s+lower_right_quarter\s*:=\s*PackedVector2Array\(\[\s*center,\s*center\s*\+\s*Vector2\(0\.0,\s*RADIUS\),\s*center\s*\+\s*Vector2\(RADIUS,\s*0\.0\),' -and $extraEnergySlot -match 'var\s+upper_left_quarter\s*:=\s*PackedVector2Array\(\[\s*center,\s*center\s*\+\s*Vector2\(0\.0,\s*-RADIUS\),\s*center\s*\+\s*Vector2\(-RADIUS,\s*0\.0\),' -and $extraPlayer -notmatch 'func\s+_draw_bonus_step_arrows\(\)' -and $extraPlayer -match 'STEP_FRAME_COLOR\s*:=\s*Color\("#38D6E8"\)' -and $extraPlayer -match 'if\s+direction\s+in\s+bonus_step_directions\s+and\s+duplicate_index\s*==\s*0:\s*\r?\n\s*outline_color\s*=\s*STEP_FRAME_COLOR' -and $extraCell -match 'func\s+set_bonus_step_highlight\(enabled:\s*bool\)' -and $extraCell -match 'Color\(0\.22,\s*0\.84,\s*0\.91,\s*0\.12\)' -and $extraBoard -match '_set_bonus_step_cell_highlights\(bonus_directions\)' -and $extraBoard -match 'func\s+_set_bonus_step_cell_highlights\(directions:\s*Array\[int\]\)' -and $extraHud -match 'Color\("#C8E64A"\)'
	},
	@{
	...
	@{
		...
		Pass = $extraHud -match 'status_vbox\.add_child\(energy_row\)(?:(?!func\s)[\s\S])*?status_vbox\.add_child\(direction_row\)(?:(?!func\s)[\s\S])*?status_vbox\.add_child\(action_row\)' -and $extraHud -match 'action_row\.add_child\(ultimate_action_label\)' -and $extraHud -match 'action_row\.add_child\(dash_action_label\)' -and $extraHud -match 'dash_action_label\.modulate\s*=\s*Color\.WHITE\s+if\s+quarter_units\s*>=\s*bonus_step_cost\s+or\s+bonus_step_armed' -and $extraHud -match 'ultimate_action_label\.modulate\s*=\s*Color\.WHITE\s+if\s+quarter_units\s*>=\s*16\s+or\s+ultimate_steps\s*>\s*0' -and $extraHud -notmatch 'Space/X:\s*Wait' -and $extraBoard -match 'BOARD_PREFERRED_SCALE\s*:=\s*1\.25' -and $extraBoard -match 'scale\s*=\s*Vector2\.ONE\s*\*\s*board_scale' -and $extraBoard -match 'BOARD_SIDEBAR_WIDTH\s*\+\s*\(content_width\s*-\s*scaled_width\)\s*\*\s*0\.5'
	},
	@{
