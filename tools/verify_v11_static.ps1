...
$checks = @(
	...
	@{
		Name = "EXTRA replaces white move and attack prompts with stored direction arrows"
		Pass = $extraBoard -match 'set_stored_direction_slots\(stored_directions,\s*inventory\.max_size\)' -and $extraBoard -match 'for\s+direction_value\s+in\s+inventory\.queue:' -and $extraPlayer -match 'func\s+set_stored_direction_slots\(directions:\s*Array,\s*max_size:\s*int\)' -and $extraPlayer -match 'func\s+_draw_stored_direction_arrows\(\)' -and $extraPlayer -match 'DUPLICATE_OFFSET' -and $extraPlayer -match 'EXPIRING_COLOR' -and $extraPlayer -match 'stored_direction_slots\.size\(\)\s*-\s*stored_direction_max_size\s*\+\s*1' -and $extraPlayer -match 'if\s+direction\s+not\s+in\s+danger_move_directions:\s*\r?\n\s*continue' -and $extraPlayer -notmatch 'func\s+_draw\(\)(?:(?!\r?\nfunc\s)[\s\S])*?_draw_attack_ready_arrows\(\)'
	},
	@{
