...
\s*score_manager\.on_kill\(cell_type\)'
	...
	@{
		Name = "EXTRA raises spawn batches from two to five at turn milestones"
		Pass = (Test-Path -LiteralPath $extraSpawnWarningSoundPath) -and $extraBoard -match 'SPAWN_CYCLE_STEPS\s*:=\s*2' -and $extraBoard -match 'SPAWNS_PER_CYCLE\s*:=\s*2' -and $extraBoard -match 'THREE_SPAWN_TURN_THRESHOLD\s*:=\s*100' -and $extraBoard -match 'FOUR_SPAWN_TURN_THRESHOLD\s*:=\s*200' -and $extraBoard -match 'FIVE_SPAWN_TURN_THRESHOLD\s*:=\s*300' -and $extraBoard -match 'func\s+get_spawns_per_cycle\(\)\s*->\s*int(?:(?!\r?\nfunc\s)[\s\S])*?survival_turns\s*>=\s*FIVE_SPAWN_TURN_THRESHOLD(?:(?!\r?\nfunc\s)[\s\S])*?return\s+5(?:(?!\r?\nfunc\s)[\s\S])*?survival_turns\s*>=\s*FOUR_SPAWN_TURN_THRESHOLD(?:(?!\r?\nfunc\s)[\s\S])*?return\s+4(?:(?!\r?\nfunc\s)[\s\S])*?survival_turns\s*>=\s*THREE_SPAWN_TURN_THRESHOLD(?:(?!\r?\nfunc\s)[\s\S])*?return\s+3' -and $extraBoard -match 'func\s+_start_new_cycle\(\)(?:(?!\r?\nfunc\s)[\s\S])*?min\(get_spawns_per_cycle\(\),\s*available\.size\(\)\)(?:(?!\r?\nfunc\s)[\s\S])*?if\s+not\s+candidate_cells\.is_empty\(\):\s*\r?\n\s*play_spawn_warning_sound\(\)'
	},
	@{
		Name = "EXTRA MCTS and Python simulators mirror turn-based spawn escalation"
		Pass = $extraSimBoard -match 'THREE_SPAWN_TURN_THRESHOLD\s*:=\s*100' -and $extraSimBoard -match 'FOUR_SPAWN_TURN_THRESHOLD\s*:=\s*200' -and $extraSimBoard -match 'FIVE_SPAWN_TURN_THRESHOLD\s*:=\s*300' -and $extraSimBoard -match 'func\s+get_spawns_per_cycle\(\)\s*->\s*int(?:(?!\r?\nfunc\s)[\s\S])*?survival_turns\s*>=\s*FIVE_SPAWN_TURN_THRESHOLD(?:(?!\r?\nfunc\s)[\s\S])*?return\s+5(?:(?!\r?\nfunc\s)[\s\S])*?survival_turns\s*>=\s*FOUR_SPAWN_TURN_THRESHOLD(?:(?!\r?\nfunc\s)[\s\S])*?return\s+4(?:(?!\r?\nfunc\s)[\s\S])*?survival_turns\s*>=\s*THREE_SPAWN_TURN_THRESHOLD(?:(?!\r?\nfunc\s)[\s\S])*?return\s+3' -and $extraCma -match 'THREE_SPAWN_TURN_THRESHOLD\s*=\s*100' -and $extraCma -match 'FOUR_SPAWN_TURN_THRESHOLD\s*=\s*200' -and $extraCma -match 'FIVE_SPAWN_TURN_THRESHOLD\s*=\s*300' -and $extraCma -match 'def\s+spawns_per_cycle\(self\)\s*->\s*int:(?:(?!\r?\n\s*def\s)[\s\S])*?self\.survival_turns\s*>=\s*FIVE_SPAWN_TURN_THRESHOLD(?:(?!\r?\n\s*def\s)[\s\S])*?return\s+5(?:(?!\r?\n\s*def\s)[\s\S])*?self\.survival_turns\s*>=\s*FOUR_SPAWN_TURN_THRESHOLD(?:(?!\r?\n\s*def\s)[\s\S])*?return\s+4(?:(?!\r?\n\s*def\s)[\s\S])*?self\.survival_turns\s*>=\s*THREE_SPAWN_TURN_THRESHOLD(?:(?!\r?\n\s*def\s)[\s\S])*?return\s+3'
	},
	@{
