...
$checks = @(
	...
	@{
		Name = "releases use simple whoosh without block movement audio"
	...
	@{
		...
		Pass = $titleScreen -match 'func\s+config_labels\(\)[\s\S]*"FULLSCREEN\s+%s"[\s\S]*"GRID\s+%s"[\s\S]*"AUDIO"[\s\S]*"BACK"' -and $titleScreen -match 'CONFIG_PANEL_OFFSET_X\s*:=\s*370\.0' -and $titleScreen -match 'func\s+config_list_center\(\)[\s\S]*center\.x\s*\+\s*CONFIG_PANEL_OFFSET_X,\s*center\.y' -and $titleScreen -match 'func\s+config_option_center\(index:\s*int\)[\s\S]*float\(index\)\s*-\s*float\(option_count\s*-\s*1\)\s*/\s*2\.0' -and $titleScreen -match 'posmod\(config_index\s*-\s*1,\s*option_count\)[\s\S]*posmod\(config_index\s*\+\s*1,\s*option_count\)' -and $titleScreen -match 'func\s+draw_audio_option\(' -and $titleScreen -match 'AUDIO_SLIDER_SIZE\.x\s*\*\s*fill_ratio' -and $titleScreen -match 'func\s+play_config_action\(' -and $titleScreen -match '"config_action_offset"[\s\S]*-CONFIG_ACTION_RETREAT[\s\S]*CONFIG_ACTION_HOLD_SECONDS[\s\S]*CONFIG_ACTION_EXTEND[\s\S]*CONFIG_ACTION_RETURN_SECONDS' -and $titleScreen -match 'is_cancel_key\(event\)[\s\S]*MenuMode\.CONFIG:[\s\S]*play_config_action\(leave_config\)[\s\S]*MenuMode\.INFO:[\s\S]*play_config_action\(leave_info\)' -and $titleScreen -match 'play_config_action\(leave_config\)' -and $titleScreen -match 'toggle_fullscreen\(\)' -and $titleScreen -match 'Campaign\.grid_lines_visible\s*=\s*not Campaign\.grid_lines_visible' -and $titleScreen -match 'Campaign\.set_audio_volume\(' -and $titleScreen -match 'func\s+leave_config\(' -and $campaign -match 'var\s+grid_lines_visible\s*:=\s*false' -and $campaign -match 'func\s+set_audio_volume\(' -and $mainEntry -match 'set_grid_lines_visible\(Campaign\.grid_lines_visible\)'
	},
	@{
