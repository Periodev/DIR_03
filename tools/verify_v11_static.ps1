...
$checks = @(
	...
	@{
		Name = "EXTRA PLN uses a three-slot rolling direction inventory plus one temporary STEP slot"
		Pass = $extraCharacterData -match '"PLN"\s*:\s*\{(?:(?!\r?\n\s*\},)[\s\S])*?"seq"\s*:\s*3' -and $extraHud -match '_max_slots\s*=\s*data\["seq"\]' -and $extraHud -match 'for\s+i\s+in\s+_max_slots\s*\+\s*Inventory\.BONUS_OVERFLOW_SLOTS' -and $extraHud -match 'if\s+i\s*>=\s*_max_slots:(?:(?!\r?\nfunc\s)[\s\S])*?slot\.visible\s*=\s*false' -and $extraHud -match 'func\s+update_inventory\(inv:\s*Inventory\)(?:(?!\r?\nfunc\s)[\s\S])*?if\s+i\s*>=\s*_max_slots:\s*\r?\n\s*slot\.visible\s*=\s*i\s*<\s*inv\.queue\.size\(\)' -and $extraBoard -match 'inventory\.setup\(char_name\)'
	},
	@{
		Name = "EXTRA STEP repositioning banks one temporary overflow direction that a normal move trims"
		Pass = $extraInventory -match 'const\s+BONUS_OVERFLOW_SLOTS\s*:=\s*1' -and $extraInventory -match 'func\s+push\(dir:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?while\s+queue\.size\(\)\s*>=\s*max_size:\s*\r?\n\s*queue\.pop_front\(\)' -and $extraInventory -match 'func\s+push_bonus\(dir:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?var\s+overflow_limit:\s*int\s*=\s*max_size\s*\+\s*BONUS_OVERFLOW_SLOTS(?:(?!\r?\nfunc\s)[\s\S])*?while\s+queue\.size\(\)\s*>=\s*overflow_limit:\s*\r?\n\s*queue\.pop_front\(\)' -and $extraInventory -match 'func\s+is_overflowing\(\)\s*->\s*bool:\s*\r?\n\s*return\s+queue\.size\(\)\s*>\s*max_size' -and $extraBoard -match 'func\s+_complete_live_move\((?:(?!\r?\nfunc\s)[\s\S])*?var\s+memory_token\s*:=\s*_get_move_memory_token\(dir\)(?:(?!\r?\nfunc\s)[\s\S])*?if\s+is_bonus_step:(?:(?!\r?\nfunc\s)[\s\S])*?inventory\.push_bonus\(memory_token\)'
	},
	@{
		Name = "EXTRA STEP attacks keep their direction token while normal attacks consume it"
		Pass = $extraBoard -match 'func\s+try_move\(dir:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?if\s+is_bonus_step:\s*\r?\n(?:(?!\r?\nfunc\s)[\s\S])*?if\s+not\s+_has_attack_direction\(dir\):\s*\r?\n\s*return\s+false(?:(?!\r?\nfunc\s)[\s\S])*?elif\s+not\s+_consume_attack_direction\(dir\):\s*\r?\n\s*return\s+false'
	},
	@{
		Name = "EXTRA HUD reads out the chain length and the next kill's energy gain"
		Pass = $extraHud -match 'func\s+update_combo\(combo:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?"COMBO %d"\s*%\s*combo' -and $extraHud -notmatch '"COMBO x%d"' -and $extraHud -match 'var\s+energy_gain_label:\s*Label' -and $extraHud -match 'func\s+update_energy_gain\(quarter_units:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?"\+%d"\s*%\s*quarter_units(?:(?!\r?\nfunc\s)[\s\S])*?ENERGY_GAIN_COLOR\s+if\s+quarter_units\s*>\s*0\s+else\s+ENERGY_GAIN_IDLE_COLOR' -and $extraBoard -match 'func\s+get_next_kill_energy_gain\(\)\s*->\s*int(?:(?!\r?\nfunc\s)[\s\S])*?ultimate_dashes_remaining\s*>\s*0\s+or\s+bonus_step_armed:\s*\r?\n\s*return\s+0' -and $extraBoard -match 'func\s+energy_gain_for_combo\(combo:\s*int\)\s*->\s*int' -and $extraBoard -match 'func\s+_charge_energy_for_combo\(combo:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?energy_gain_for_combo\(combo\)' -and $extraMain -match 'hud\.update_energy_gain\(board\.get_next_kill_energy_gain\(\)\)'
	},
	@{
		Name = "EXTRA counts every chain link but saturates what a link pays"
		Pass = $extraScoreManager -match 'const\s+MAX_COMBO_TIER\s*:=\s*6' -and $extraScoreManager -match 'const\s+COMBO_SCORE_MULTIPLIERS\s*:=\s*\[1,\s*2,\s*3,\s*5,\s*10,\s*20\]' -and $extraScoreManager -match 'func\s+combo_tier\(combo:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?clampi\(combo,\s*1,\s*MAX_COMBO_TIER\)' -and $extraScoreManager -match 'func\s+combo_multiplier\(combo:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?COMBO_SCORE_MULTIPLIERS\[combo_tier\(combo\)\s*-\s*1\]' -and $extraScoreManager -match 'var\s+points:\s*int\s*=\s*base\s*\*\s*combo_multiplier\(combo_counter\)' -and $extraScoreManager -match 'func\s+advance_combo\(\)(?:(?!\r?\nfunc\s)[\s\S])*?combo_counter\s*\+=\s*1' -and $extraBoard -match 'score_manager\.advance_combo\(\)' -and $extraBoard -match 'func\s+energy_gain_for_combo\(combo:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?combo\s*>=\s*6:' -and $extraComboBot -match 'func\s+_combo_payout\(combo:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?clampi\(combo,\s*1,\s*ScoreManager\.MAX_COMBO_TIER\)' -and $extraComboBot -notmatch 'combo\s*\*\s*combo'
	},
	@{
		Name = "EXTRA energy income only happens on turns that advance the spawn clock"
		Pass = $extraBoard -match 'var\s+_bonus_step_kill_active:\s*bool\s*=\s*false' -and $extraBoard -match 'func\s+try_move\(dir:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?_bonus_step_kill_active\s*=\s*is_bonus_step\s*\r?\n\s*_resolve_attack\(dir,\s*target,\s*target_type\)\s*\r?\n\s*_bonus_step_kill_active\s*=\s*false' -and $extraBoard -match 'func\s+_kill_flow\((?:(?!\r?\nfunc\s)[\s\S])*?if\s+not\s+_ultimate_chain_started\s+and\s+not\s+_bonus_step_kill_active:\s*\r?\n\s*_charge_energy_for_combo' -and $extraBoard -match 'func\s+restart\(\)(?:(?!\r?\nfunc\s)[\s\S])*?_bonus_step_kill_active\s*=\s*false' -and $extraBoard -match 'func\s+get_bonus_step_cost\(\)\s*->\s*int:\s*\r?\n\s*return\s+ENERGY_SLOT_COST' -and $extraHud -match 'func\s+update_energy\((?:(?!\r?\nfunc\s)[\s\S])*?bonus_step_cost:\s*int(?:(?!\r?\nfunc\s)[\s\S])*?quarter_units\s*>=\s*bonus_step_cost' -and $extraMain -match 'board\.get_bonus_step_cost\(\)'
	},
	@{
		Name = "EXTRA spends one full energy slot to arm a free combo-preserving bonus step"
		Pass = $extraBoard -match 'ENERGY_QUARTER_UNITS_MAX\s*:=\s*16' -and $extraBoard -match 'ENERGY_SLOT_COST\s*:=\s*4' -and $extraBoard -match 'func\s+try_energy_bonus_step\(\)(?:(?!\r?\nfunc\s)[\s\S])*?energy_quarter_units\s*-=\s*cost(?:(?!\r?\nfunc\s)[\s\S])*?bonus_step_armed\s*=\s*true' -and $extraBoard -match 'func\s+try_move\(dir:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?var\s+is_bonus_step:\s*bool\s*=\s*bonus_step_armed(?:(?!\r?\nfunc\s)[\s\S])*?_complete_live_move\(dir,\s*target,\s*is_bonus_step\)' -and $extraBoard -match 'func\s+_complete_live_move\(dir:\s*int,\s*target:\s*Vector2i,\s*is_bonus_step:\s*bool\)(?:(?!\r?\nfunc\s)[\s\S])*?elif\s+not\s+_will_spawn_hit_target_this_turn\(target\):(?:(?!\r?\nfunc\s)[\s\S])*?score_manager\.on_move_to_live\(\)(?:(?!\r?\nfunc\s)[\s\S])*?_finalize_turn_after_action\(true,\s*false\)' -and $extraBoard -match 'func\s+energy_gain_for_combo\(combo:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?1:\s*\r?\n\s*return\s+1(?:(?!\r?\nfunc\s)[\s\S])*?2,\s*3:\s*\r?\n\s*return\s+2(?:(?!\r?\nfunc\s)[\s\S])*?4,\s*5:\s*\r?\n\s*return\s+4(?:(?!\r?\nfunc\s)[\s\S])*?combo\s*>=\s*6:\s*\r?\n\s*return\s+8' -and $extraMain -match 'KEY_X(?:(?!\r?\n\s*#)[\s\S])*?try_energy_bonus_step\(\)' -and $extraBoard -notmatch 'var\s+ultimate_ready:\s*bool\s*=\s*false'
	},
	@{
		Name = "EXTRA consumes full energy on Z to activate the four-dash ULT"
		Pass = $extraBoard -match 'ULT_DASH_COUNT\s*:=\s*4' -and $extraBoard -match 'func\s+try_energy_ultimate\(\)(?:(?!\r?\nfunc\s)[\s\S])*?energy_quarter_units\s*<\s*ENERGY_QUARTER_UNITS_MAX(?:(?!\r?\nfunc\s)[\s\S])*?energy_quarter_units\s*=\s*0(?:(?!\r?\nfunc\s)[\s\S])*?ultimate_dashes_remaining\s*=\s*ULT_DASH_COUNT' -and $extraBoard -notmatch 'func\s+try_energy_ultimate\(\)(?:(?!\r?\nfunc\s)[\s\S])*?score_manager\.reset_combo\(\)' -and $extraBoard -match 'func\s+_try_ultimate_dash\(dir:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?if\s+destination\s*==\s*origin:\s*\r?\n\s*return\s+false(?:(?!\r?\nfunc\s)[\s\S])*?_consume_attack_direction\(dir\)' -and $extraBoard -match 'func\s+_try_ultimate_dash\(dir:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?player_pos\s*=\s*destination\s*\r?\n\s*inventory\.push\(dir\)' -and $extraBoard -match 'var\s+freeze_spawn_cycle:\s*bool\s*=\s*ultimate_dashes_remaining\s*>\s*0(?:(?!\r?\nfunc\s)[\s\S])*?_finalize_turn_after_action\(freeze_spawn_cycle\)' -and $extraBoard -notmatch 'func\s+_try_ultimate_dash\(dir:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?score_manager\.reset_combo\(\)' -and $extraBoard -match 'if\s+not\s+_ultimate_chain_started\s+and\s+not\s+_bonus_step_kill_active:\s*\r?\n\s*_charge_energy_for_combo' -and $extraMain -match 'KEY_Z(?:(?!\r?\n\s*#)[\s\S])*?try_energy_ultimate\(\)' -and $extraMain -notmatch 'KEY_X(?:(?!\r?\n\s*#)[\s\S])*?try_wait\(\)'
	},
	@{
...
func\s)[\s\S])*?var\s+center\s*:=\s*info_back_center\(panel_center\)'
	...
	@{
		...
		Pass = $extraHud -match 'var\s+energy_slots:\s*Array\[DIRExtraEnergySlot\]' -and $extraHud -match 'for\s+_i\s+in\s+4:' -and $extraHud -match 'func\s+update_energy\((?:(?!\r?\nfunc\s)[\s\S])*?clampi\(quarter_units\s*-\s*i\s*\*\s*4,\s*0,\s*4\)(?:(?!\r?\nfunc\s)[\s\S])*?float\(slot_quarter_units\)\s*/\s*4\.0' -and $extraEnergySlot -match 'class_name\s+DIRExtraEnergySlot' -and $extraEnergySlot -match 'var\s+lower_right_quarter\s*:=\s*PackedVector2Array\(\[\s*center,\s*center\s*\+\s*Vector2\(0\.0,\s*RADIUS\),\s*center\s*\+\s*Vector2\(RADIUS,\s*0\.0\),' -and $extraEnergySlot -match 'var\s+upper_left_quarter\s*:=\s*PackedVector2Array\(\[\s*center,\s*center\s*\+\s*Vector2\(0\.0,\s*-RADIUS\),\s*center\s*\+\s*Vector2\(-RADIUS,\s*0\.0\),' -and $extraEnergySlot -match 'OUTLINE_COLOR\s*:=\s*Color\("#4A5058"\)' -and $extraEnergySlot -match 'FILL_COLOR\s*:=\s*Color\("#2FD9A0"\)' -and $extraEnergySlot -match 'FULL_FILL_COLOR\s*:=\s*Color\("#3BE8B4"\)' -and $extraEnergySlot -match 'FULL_OUTLINE_COLOR\s*:=\s*Color\("#DFFFE9"\)' -and $extraEnergySlot -match 'FULL_OUTLINE_WIDTH\s*:=\s*1\.5' -and $extraPlayer -notmatch 'func\s+_draw_bonus_step_arrows\(\)' -and $extraPlayer -match 'STEP_FRAME_COLOR\s*:=\s*Color\("#2FD9A0"\)' -and $extraPlayer -match 'if\s+direction\s+in\s+bonus_step_directions\s+and\s+duplicate_index\s*==\s*0:\s*\r?\n\s*outline_color\s*=\s*STEP_FRAME_COLOR' -and $extraCell -match 'func\s+set_bonus_step_highlight\(enabled:\s*bool\)' -and $extraCell -match 'BONUS_STEP_HIGHLIGHT_COLOR\s*:=\s*Color\(0\.1843137,\s*0\.8509804,\s*0\.6274510,\s*0\.12\)' -and $extraBoard -match '_set_bonus_step_cell_highlights\(bonus_directions\)' -and $extraBoard -match 'func\s+_set_bonus_step_cell_highlights\(directions:\s*Array\[int\]\)' -and $extraHud -match 'STEP_AVAILABLE_COLOR\s*:=\s*Color\("#2FD9A0"\)' -and $extraHud -match 'STEP_UNAVAILABLE_COLOR\s*:=\s*Color\("#1E7A5C"\)' -and $extraSlashEffect -match 'SLASH_CORE_COLOR\s*:=\s*Color\("#EAFFF0"\)' -and $extraSlashEffect -match 'SLASH_OUTER_COLOR\s*:=\s*Color\("#33CC4D"\)'
	},
	@{
	...
	@{
		...
		Pass = $extraScoreManager -match 'var\s+max_combo:\s*int\s*=\s*0' -and $extraScoreManager -match 'max_combo\s*=\s*maxi\(max_combo,\s*combo_counter\)' -and $extraScoreManager -match 'func\s+reset\(\)(?:(?!\r?\nfunc\s)[\s\S])*?max_combo\s*=\s*0' -and $extraHud -match 'func\s+show_game_over\(final_score:\s*int,\s*max_combo:\s*int\)(?:(?!\r?\nfunc\s)[\s\S])*?"MAX COMBO %d"\s*%\s*max_combo' -and $extraHud -notmatch 'MAX COMBO x' -and $extraMain -match 'hud\.show_game_over\(final_score,\s*board\.score_manager\.max_combo\)'
	},
	@{
		Name = "EXTRA uses a left three-row status panel and an enlarged adaptive board"
		Pass = $extraHud -match 'status_vbox\.add_child\(energy_row\)(?:(?!func\s)[\s\S])*?status_vbox\.add_child\(direction_row\)(?:(?!func\s)[\s\S])*?status_vbox\.add_child\(action_row\)' -and $extraHud -match 'action_row\.add_child\(ultimate_action_label\)' -and $extraHud -match 'action_row\.add_child\(dash_action_label\)' -and $extraHud -match 'var\s+step_available:\s*bool\s*=\s*quarter_units\s*>=\s*bonus_step_cost\s+or\s+bonus_step_armed' -and $extraHud -match 'STEP_AVAILABLE_COLOR\s+if\s+step_available\s+else\s+STEP_UNAVAILABLE_COLOR' -and $extraHud -match 'ultimate_action_label\.modulate\s*=\s*Color\.WHITE\s+if\s+quarter_units\s*>=\s*16\s+or\s+ultimate_steps\s*>\s*0' -and $extraHud -notmatch 'Space/X:\s*Wait' -and $extraBoard -match 'BOARD_PREFERRED_SCALE\s*:=\s*1\.25' -and $extraBoard -match 'scale\s*=\s*Vector2\.ONE\s*\*\s*board_scale' -and $extraBoard -match 'BOARD_SIDEBAR_WIDTH\s*\+\s*\(content_width\s*-\s*scaled_width\)\s*\*\s*0\.5'
	},
	@{
