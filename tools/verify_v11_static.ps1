...
func\s)[\s\S])*?var\s+center\s*:=\s*info_back_center\(panel_center\)'
	...
	@{
		...
		Pass = $extraHud -match 'status_vbox\.add_child\(energy_row\)(?:(?!func\s)[\s\S])*?status_vbox\.add_child\(direction_row\)(?:(?!func\s)[\s\S])*?status_vbox\.add_child\(action_row\)' -and $extraHud -match 'action_row\.add_child\(ultimate_action_label\)' -and $extraHud -match 'action_row\.add_child\(dash_action_label\)' -and $extraHud -notmatch 'VSeparator\.new\(\)' -and $extraHud -match 'DIRECTION_ACTIVE_COLOR\s*:=\s*Color\("#7FE85A"\)' -and $extraHud -match 'DIRECTION_EXPIRING_COLOR\s*:=\s*Color\("#ADDEB7"\)' -and $extraHud -match 'DIRECTION_EMPTY_COLOR\s*:=\s*Color\("#4A5058"\)' -and $extraHud -match 'DASH_AVAILABLE_COLOR\s*:=\s*Color\("#47EB7A"\)' -and $extraPlayer -match 'ACTIVE_COLOR\s*:=\s*Color\("#7FE85A"\)' -and $extraCell -match 'attack_color\s*:=\s*Color\("#7FE85A"\)' -and $extraHud -match 'var\s+step_available:\s*bool\s*=\s*quarter_units\s*>=\s*bonus_step_cost\s+or\s+bonus_step_armed' -and $extraHud -match 'STEP_AVAILABLE_COLOR\s+if\s+step_available\s+else\s+STEP_UNAVAILABLE_COLOR' -and $extraHud -match 'var\s+ultimate_available:\s*bool\s*=\s*quarter_units\s*>=\s*16\s+or\s+ultimate_steps\s*>\s*0' -and $extraHud -match 'DASH_AVAILABLE_COLOR\s+if\s+ultimate_available\s+else\s+DIRECTION_EMPTY_COLOR' -and $extraHud -notmatch 'Space/X:\s*Wait' -and $extraBoard -match 'BOARD_PREFERRED_SCALE\s*:=\s*1\.25' -and $extraBoard -match 'scale\s*=\s*Vector2\.ONE\s*\*\s*board_scale' -and $extraBoard -match 'BOARD_SIDEBAR_WIDTH\s*\+\s*\(content_width\s*-\s*scaled_width\)\s*\*\s*0\.5'
	},
	@{
