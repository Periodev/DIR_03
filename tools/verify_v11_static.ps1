...
$checks = @(
	...
	@{
		...
		Pass = $extraHud -match 'ultimate_action_label\.text\s*=\s*"\[Z\] DASH"(?:(?!var\s+q_label)[\s\S])*?inv_hbox\.add_child\(ultimate_action_label\)(?:(?!var\s+q_label)[\s\S])*?dash_action_label\.text\s*=\s*"\[X\] STEP"(?:(?!var\s+q_label)[\s\S])*?inv_hbox\.add_child\(dash_action_label\)(?:(?!energy_container)[\s\S])*?var\s+q_label' -and $extraHud -match 'dash_action_label\.modulate\s*=\s*Color\.WHITE\s+if\s+quarter_units\s*>=\s*bonus_step_cost\s+or\s+bonus_step_armed' -and $extraHud -match 'ultimate_action_label\.modulate\s*=\s*Color\.WHITE\s+if\s+quarter_units\s*>=\s*16\s+or\s+ultimate_steps\s*>\s*0' -and $extraHud -notmatch 'Space/X:\s*Wait'
	},
	@{
