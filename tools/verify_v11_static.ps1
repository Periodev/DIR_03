...
$checks = @(
	...
	@{
		Name = "Fin reveals thanks only after completion presentation"
		Pass = $playerInterface -match 'FINAL_LEVEL_ID\s*:=\s*"3-12"' -and $playerInterface -match '"THANK YOU FOR PLAYING"' -and $playerInterface -match 'func\s+finish_completion_presentation\(\)[\s\S]*Campaign\.active_level_id\s*!=\s*FINAL_LEVEL_ID[\s\S]*final_completion_revealed\s*=\s*true' -and $playerInterface -match 'func\s+refresh_final_completion_message\(\)[\s\S]*if\s+not\s+game_board\.level_completed:[\s\S]*final_completion_revealed\s*=\s*false[\s\S]*goals_title\.visible\s*=\s*not\s+show_message[\s\S]*goals_value\.visible\s*=\s*not\s+show_message' -and $playerBoardView -match 'func\s+finish_completion_feedback\(\)[\s\S]*game_board\.finish_completion_presentation\(\)' -and $gameBoard -match 'func\s+finish_completion_presentation\(\)[\s\S]*game_hud\.finish_completion_presentation\(\)'
	},
	@{
...
\s*score_manager\.on_kill\(cell_type\)'
	...
	@{
		Name = "release flag disables every debug function key while preserving F1 help"
		Pass = $campaign -match 'const\s+DEBUG_SHORTCUTS_ENABLED\s*:=\s*false' -and $worldMap -match 'func\s+is_unlock_all_key\(event:\s*InputEvent\)\s*->\s*bool:\s*\r?\n\s*if\s+not\s+Campaign\.DEBUG_SHORTCUTS_ENABLED:\s*\r?\n\s*return\s+false' -and $classicLevelSelect -match 'func\s+is_unlock_all_key\(event:\s*InputEvent\)\s*->\s*bool:\s*\r?\n\s*if\s+not\s+Campaign\.DEBUG_SHORTCUTS_ENABLED:\s*\r?\n\s*return\s+false' -and $classicLevelSelect -match 'func\s+is_complete_selected_key\(event:\s*InputEvent\)\s*->\s*bool:\s*\r?\n\s*if\s+not\s+Campaign\.DEBUG_SHORTCUTS_ENABLED:\s*\r?\n\s*return\s+false' -and $mainEntry -match 'func\s+is_quick_complete_key\(event:\s*InputEvent\)\s*->\s*bool:\s*\r?\n\s*if\s+not\s+Campaign\.DEBUG_SHORTCUTS_ENABLED:\s*\r?\n\s*return\s+false' -and $extraMain -match 'Campaign\.DEBUG_SHORTCUTS_ENABLED\s+and\s+keycode\s*==\s*KEY_F2' -and $extraMain -match 'Campaign\.DEBUG_SHORTCUTS_ENABLED\s+and\s+keycode\s*==\s*KEY_F3' -and $extraMain -match 'Campaign\.DEBUG_SHORTCUTS_ENABLED\s+and\s+keycode\s*==\s*KEY_F4' -and $extraMain -match 'Campaign\.DEBUG_SHORTCUTS_ENABLED\s+and\s+keycode\s*==\s*KEY_F5' -and $extraMain -match 'Campaign\.DEBUG_SHORTCUTS_ENABLED\s+and\s+keycode\s*==\s*KEY_F6' -and $extraMain -match 'if\s+keycode\s*==\s*KEY_F1:\s*\r?\n\s*_toggle_help\(\)' -and $extraHud -match 'ai_status_label\.visible\s*=\s*Campaign\.DEBUG_SHORTCUTS_ENABLED'
	},
	@{
...
func\s)[\s\S])*?var\s+center\s*:=\s*info_back_center\(panel_center\)'
	...
	@{
		...
		Pass = $extraHud -match 'signal\s+nav_action_requested\(action:\s*StringName\)' -and $extraHud -match 'NAV_COMMAND_TITLE' -and $extraHud -match 'NAV_COMMAND_HELP' -and $extraHud -match 'NAV_COMMAND_RESTART' -and $extraHud -match 'func\s+_nav_entry\(key:\s*String,\s*action:\s*String,\s*command:\s*StringName\)' -and $extraHud -match 'CURSOR_POINTING_HAND' -and $extraHud -match 'entry\.gui_input\.connect' -and $extraHud -match 'MOUSE_BUTTON_LEFT' -and $extraHud -match 'nav_action_requested\.emit\(command\)' -and $extraHud -match 'const\s+NAV_KEY_COLOR' -and $extraHud -match 'const\s+NAV_ACTION_COLOR' -and $extraHud -match 'action_label\.add_theme_font_size_override\("font_size",\s*22\)' -and $extraHud -match 'NAV_ROW_WIDTH\s*:=\s*520\.0' -and $extraHud -notmatch 'nav_clearance|HELP_PANEL_TOP' -and $extraHud -match '\(viewport_size\.y\s*-\s*help_size\.y\)\s*\*\s*0\.5' -and $extraHud -match 'func\s+_set_help_visible\(show_help:\s*bool\)(?:(?!\r?\nfunc\s)[\s\S])*?"BACK"\s+if\s+show_help\s+else\s+"HELP"' -and $extraHud -match 'SCORE_ROW_TOP\s*:=\s*108\.0' -and $extraHud -match 'func\s+toggle_help\(\)' -and $extraHud -match 'func\s+is_help_visible\(\)' -and $extraMain -match 'hud\.nav_action_requested\.connect\(_on_nav_action_requested\)' -and $extraMain -match 'func\s+_on_nav_action_requested\(action:\s*StringName\)' -and $extraMain -match '&"title":\s*\r?\n\s*_return_to_title\(\)' -and $extraMain -match '&"help":\s*\r?\n\s*_toggle_help\(\)' -and $extraMain -match '&"restart":\s*\r?\n\s*_restart_game\(\)' -and $extraMain -match 'if\s+hud\.is_help_visible\(\):\s*\r?\n\s*if\s+keycode\s+in\s+\[KEY_ESCAPE,\s*KEY_F1,\s*KEY_SPACE\]:(?:(?!\r?\n\s*if\s+keycode\s*==\s*KEY_ESCAPE)[\s\S])*?_toggle_help\(\)' -and $extraMain -match 'func\s+_process\(delta:\s*float\)(?:(?!\r?\nfunc\s)[\s\S])*?if\s+hud\.is_help_visible\(\):\s*\r?\n\s*return'
	},
	@{
