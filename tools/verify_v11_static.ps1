...
func\s)[\s\S])*?var\s+center\s*:=\s*info_back_center\(panel_center\)'
	...
	@{
		...
		Pass = $extraHud -match 'var\s+energy_slots:\s*Array\[DIRExtraEnergySlot\]' -and $extraHud -match 'for\s+_i\s+in\s+4:' -and $extraHud -match 'func\s+update_energy\((?:(?!\r?\nfunc\s)[\s\S])*?clampi\(quarter_units\s*-\s*i\s*\*\s*4,\s*0,\s*4\)(?:(?!\r?\nfunc\s)[\s\S])*?float\(slot_quarter_units\)\s*/\s*4\.0' -and $extraEnergySlot -match 'class_name\s+DIRExtraEnergySlot' -and $extraEnergySlot -match 'var\s+lower_right_quarter\s*:=\s*PackedVector2Array\(\[\s*center,\s*center\s*\+\s*Vector2\(0\.0,\s*RADIUS\),\s*center\s*\+\s*Vector2\(RADIUS,\s*0\.0\),' -and $extraEnergySlot -match 'class_name\s+DIRExtraEnergySlot' -and $extraEnergySlot -match 'var\s+upper_left_quarter\s*:=\s*PackedVector2Array\(\[\s*center,\s*center\s*\+\s*Vector2\(0\.0,\s*-RADIUS\),\s*center\s*\+\s*Vector2\(-RADIUS,\s*0\.0\),' -and $extraPlayer -notmatch 'func\s+_draw_bonus_step_arrows\(\)' -and $extraPlayer -match 'STEP_FRAME_COLOR\s*:=\s*Color\("#C8E64A"\)' -and $extraPlayer -match 'if\s+direction\s+in\s+bonus_step_directions\s+and\s+duplicate_index\s*==\s*0:\s*\r?\n\s*outline_color\s*=\s*STEP_FRAME_COLOR' -and $extraHud -match 'Color\("#C8E64A"\)'
	},
	@{
