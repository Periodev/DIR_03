...
$checks = @(
	...
	@{
		Name = "EXTRA HUD shows six heat tiers and the next kill's energy gain"
...
func\s)[\s\S])*?var\s+center\s*:=\s*info_back_center\(panel_center\)'
	...
	@{
		Name = "EXTRA HUD emphasizes score and heat without break or turn labels"
