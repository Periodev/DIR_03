...
\s*score_manager\.on_kill\(cell_type\)'
	...
	@{
		Name = "EXTRA adds zero to two three-turn spawns after 10000 points"
		Pass = $extraBoard -match 'DELAYED_SPAWN_SCORE_THRESHOLD\s*:=\s*10000' -and $extraBoard -match 'DELAYED_SPAWN_MAX_PER_CYCLE\s*:=\s*2' -and $extraBoard -match 'DELAYED_SPAWN_COUNTDOWN\s*:=\s*2' -and $extraBoard -match 'score_manager\.score\s*>=\s*DELAYED_SPAWN_SCORE_THRESHOLD' -and $extraBoard -match 'randi_range\(0,\s*DELAYED_SPAWN_MAX_PER_CYCLE\)' -and $extraBoard -match 'func\s+_advance_delayed_candidates\(' -and $extraSimBoard -match 'run_score\s*\+\s*score\s*>=\s*DELAYED_SPAWN_SCORE_THRESHOLD' -and $extraSimBoard -match 'rng\.randi_range\(0,\s*DELAYED_SPAWN_MAX_PER_CYCLE\)' -and $extraCma -match 'self\.score\s*>=\s*DELAYED_SPAWN_SCORE_THRESHOLD' -and $extraCma -match 'self\.rng\.randint\(0,\s*DELAYED_SPAWN_MAX_PER_CYCLE\)' -and $extraCell -match 'if\s+candidate_phase\s*==\s*1:\s*\r?\n\s*warning_color\.a\s*=\s*0\.48'
	},
	@{
