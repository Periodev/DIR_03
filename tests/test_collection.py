...
class CollectionTests(unittest.TestCase):
    def test_loads_all_first_area_levels_in_order(self) -> None:
        ...
        self.assertEqual(entries[-2].name, "反轉")
        self.assertEqual(entries[-1].name, "發條")
    ...
    def test_loads_current_third_area_lock_sequence(self) -> None:
        ...
        self.assertEqual(entries[-3].name, "長廊")
        self.assertEqual(entries[-2].name, "三相鎖")
        self.assertEqual(entries[-1].name, "Fin")
